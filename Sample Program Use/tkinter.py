...
GPIO.setup(servo1, GPIO.OUT)        # Set pinmode of servo and make it output
pwm1=GPIO.PWM(servo1, 50)            # Set the Hz of pwm pinouts
pwm1.start(0)                    # always start 0hz
...
def animate(i):
    ...
    if cardVar.get() != "" :
        ...
        # ****************************************************
        # ********** P A P E R *******************************
        paperData = open('/home/pi/Desktop/sourceCode/EarthJuan/'+ rfidVar.get() +'/can.txt',"r").read()
        ...
        p.plot()
        b.plot()
    elif guestNumberVar.get() != "":
        print "guest"
        # ****************************************************
        # ********** B O T T L E *****************************
        bottleData = open('/home/pi/Desktop/sourceCode/GuestEarthJuan/'+ guestNumberVar.get() +'/bottle.txt',"r").read()
        dataList = bottleData.split('\n')
        bdayList = []        # Number of Days
        bottleList = []      # Number of Papers
        for eachLine in dataList:
        ...
        # ****************************************************
        # ********** P A P E R *******************************
        paperData = open('/home/pi/Desktop/sourceCode/GuestEarthJuan/'+ guestNumberVar.get() +'/can.txt',"r").read()
        dataList = paperData.split('\n')
        pdayList = []        # Number of Days
        paperList = []      # Number of Papers
        for eachLine in dataList:
...
class SampleApp(Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.frames = {}
        for F in (StartPage, Choose, CreateAccount, Guest, MemberLogin, MemberData, MemAddtrash, Guestdata):      # FRAME *************
